...
export function usePipecat(): UsePipecat {
  ...
  const serverMessageHandlersRef = useRef<Set<ServerMessageHandler>>(new Set());
  ...
  const onServerMessage = useCallback((handler: ServerMessageHandler) => {
    serverMessageHandlersRef.current.add(handler);
    return () => {
      serverMessageHandlersRef.current.delete(handler);
    };
  }, []);
